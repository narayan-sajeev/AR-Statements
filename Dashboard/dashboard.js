/* ================= AR Dashboard JS (light-only) — Refactored =================
   Goal: same behavior for end users, better structure/organization/flow.
   Notes:
   - Pie slice clicks set ONLY an aging-bucket filter (still shows companies).
   - Bar clicks (balances/risk) set a customer filter (switches to invoice view for that customer).
   - The "Totals only" toggle is visible and effective ONLY in the default view (no filters).
============================================================================= */
...
window.DEFAULT_TOTALS_ONLY = false; // default-view-only user preference for totals mode
...
const ARU = (() => {
    ...
    const isDefaultView = () => !window.ACTIVE_CUSTOMER && !window.ACTIVE_BUCKET;
    ...
    /** Keep only datasets that have any non-zero values, else fallback to original */
    const filterZeroBucketsForStacked = (datasets) => {
        const filtered = (datasets || []).filter(ds => (ds.data || []).some(v => Number(v) > 0));
        return filtered.length ? filtered : datasets;
    };

    const sum = (arr) => arr.reduce((a, b) => a + Number(b || 0), 0);
    const sumObjProp = (arr, prop) => arr.reduce((a, r) => a + Number(r?.[prop] || 0), 0);

    const totalsPerItem = (items, buckets, dataMap) => items.map((_, idx) => buckets.reduce((s, b) => s + Number((dataMap[b] || [])[idx] || 0), 0));
...
/* ==========================================================================
 * Module: UI (banner/badges/title/toggle)
 * ========================================================================== */
const ARUI = (() => {
    function updateFilterBanner() {
        ...
        const label = ARU.filterBannerText();
        if (label) {
    ...
    function updateBadges(payload) {
        ...
        if (overdueBadge) overdueBadge.textContent = `${shownCount} ${noun}, ${ARU.money(shownOverdueTotal)}`;

        // Balances badge reflects shown bars (not global totals)
        ...
        if (el && payload.cust_bucket) {
            ...
            const totals = ARU.totalsPerItem(items, buckets, dataMap);
...
const ARCHART = (() => {
    function buildBalancesBar(ctx, bucketData, mode) {
        ...
        // Click handler shared behavior
        function onBarClick(nameGetter) {
            return (evt, elems) => {
                if (!elems || !elems.length) return;
                // If already in invoice mode (customer filter active), a bar click CLEARs filters
                if (window.ACTIVE_CUSTOMER) {
                    clearAllFilters();
                    return;
                }
                const name = nameGetter(elems[0].index);
                if (window.ACTIVE_CUSTOMER === name) clearAllFilters(); else setCustomerFilter(name);
            };
        }

        if (mode === 'totals') {
            const totals = items.map((_, idx) => window.AR_BUCKETS.reduce((s, b) => s + Number((bucketData.data[b] || [])[idx] || 0), 0));
                ...
                type: 'bar', data: {
                    ...
                    plugins: {
                        legend: {display: false}, tooltip: {
                            callbacks: {
                                title: (it) => items[it[0].dataIndex], label: (it) => `Total: ${ARU.money(it.parsed.y)}`
                            }
                        }
                    },
                    scales: {x: {ticks: {display: false}}, y: {ticks: {callback: v => ARU.money(v)}}},
                    onClick: onBarClick((i) => items[i]),
                }
            });
        }

        // Stacked by aging bucket — consistent colors per bucket; hide zero-only buckets
        ...
        const datasets = ARU.filterZeroBucketsForStacked(datasetsRaw);
                ...
                responsive: true, maintainAspectRatio: false, plugins: {
                    legend: {position: 'bottom'}, tooltip: {
                        callbacks: {
                            ...
                            label: (it) => `${it.dataset.label}: ${ARU.money(it.parsed.y)}`
                        }
                    }
                }, scales: {
                    x: {ticks: {display: false}, stacked: true},
                    y: {ticks: {callback: v => ARU.money(v)}, stacked: true}
                }, onClick: onBarClick((i) => items[i]),
            }
        });
    }
        ...
        return new Chart(ctx, {
            type: 'pie', data: {labels: flabels, datasets: [{data: fdata, backgroundColor: colors}]}, options: {
                responsive: true, maintainAspectRatio: false, plugins: {
                    ...
                    tooltip: {callbacks: {label: (i) => `${i.label}: ${ARU.money(i.parsed)}`}}
                }, onClick: (evt, elems) => {
                    if (!elems || !elems.length) return;
                    const bucket = flabels[elems[0].index];
                    // Always set ONLY bucket filter (clears customer filter)
                    if (window.ACTIVE_BUCKET === bucket && !window.ACTIVE_CUSTOMER) clearAllFilters(); else setBucketFilter(bucket);
                }
            }
        });
    }
    ...
    function buildRiskBar(ctx, riskTop) {
        ...
        return new Chart(ctx, {
            type: 'bar', data: {
                ...
                responsive: true,
                maintainAspectRatio: false,
                plugins: {legend: {display: false}, tooltip: {callbacks: {label: (i) => ARU.money(i.parsed.x)}}},
                scales: {x: {ticks: {callback: v => ARU.money(v)}}},
                onClick: (evt, elems) => {
                    if (!elems || !elems.length) return;
                    if (window.ACTIVE_CUSTOMER) {
                        clearAllFilters();
    ...
    return {buildBalancesBar, buildPie, buildRiskBar};
})();
...
const ARAGG = (() => {
    ...
    function aggregateFromDetail(detail) {
        ...
        // overdue risk: overdue only, top 15 customers
    ...
    return {aggregateFromDetail, buildInvoiceChartsForCustomer};
})();
...
function buildAll(payload) {
    ...
    if (kpiTotal) kpiTotal.textContent = ARU.money(payload.totals.total_ar);
    if (kpiCurrent) kpiCurrent.textContent = ARU.money(payload.totals.current_total);
    if (kpiOverdue) kpiOverdue.textContent = ARU.money(payload.totals.overdue_total);
    if (kpiOver90) kpiOver90.textContent = ARU.money(payload.totals.over_90);

    // badges
    ARUI.updateBadges(payload);

    // destroy old charts
    ARU.safeDestroy(window._stacked);
    ARU.safeDestroy(window._pie);
    ARU.safeDestroy(window._risk);

    // totals toggle (default view only)
    const toggleState = ARUI.syncTotalsToggle((newMode) => {
        // Rebuild balances bar only, using current payload
        ARU.safeDestroy(window._stacked);
        window._stacked = ARCHART.buildBalancesBar(document.getElementById('stackedBar'), window.CURRENT_PAYLOAD.cust_bucket, newMode);
    });

    // build charts
    window._stacked = ARCHART.buildBalancesBar(document.getElementById('stackedBar'), payload.cust_bucket, toggleState.mode);
    window._pie = ARCHART.buildPie(document.getElementById('agingPie'), payload.aging_summary);
    window._risk = ARCHART.buildRiskBar(document.getElementById('riskBar'), payload.risk_top);

    // tables
    ARTABLE.fillAgingTable(document.getElementById('agingTable'), payload.aging_summary, payload.totals.total_ar);
    ARTABLE.buildDetailTable(payload.invoice_detail);

    // banner
    ARUI.updateFilterBanner();
}

/* ==========================================================================
 * Module: Filters (public API functions preserved)
 * ========================================================================== */
...
function setCustomerFilter(customerName) {
    ...
    if (!customerName || window.ACTIVE_CUSTOMER === customerName) {
        clearAllFilters();
        return;
    }
    ...
    const rows = window.ORIGINAL_PAYLOAD.invoice_detail.filter(r => (r.Customer || '') === customerName);

    const payload = ARAGG.aggregateFromDetail(rows);
    const charts = ARAGG.buildInvoiceChartsForCustomer(rows);
...
function setBucketFilter(bucketName) {
    ...
    if (!bucketName || window.ACTIVE_BUCKET === bucketName) {
        clearAllFilters();
        return;
    }

    window.ACTIVE_BUCKET = bucketName;
    const rows = window.ORIGINAL_PAYLOAD.invoice_detail.filter(r => (r['Aging Bucket'] || '') === bucketName);
    ...
    if (badge) {
        badge.classList.remove('d-none');
        badge.textContent = `Filter: ${ARU.formatBucketLabel(bucketName)} ×`;
        badge.title = 'Click to clear filter';
    }
    ARUI.updateFilterBanner();
    buildAll(payload);
...
function toPayloadFromCSV(rows) {
    ...
    return ARAGG.aggregateFromDetail(detail);
}
